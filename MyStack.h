...
void Stk_add (Stack_t * stk0)
{
	...
	if ((* stk0)->count < 1)
	{
		printf ("ERROR in function Stk_add: impossible to add less than 2 numbers\n");
		assert (0);
	}

	numb1 = Stk_pop (stk0);
	numb2 = Stk_pop (stk0);
	Stk_push (stk0, numb1 + numb2);

	Stk_tester (stk0, "Stk_add");
}

void Stk_mul (Stack_t * stk0)
{
	double numb1 = 0, numb2 = 0;
	...
	if ((* stk0)->count < 1)
	{
		printf ("ERROR in function Stk_mul: impossible to mul less than 2 numbers\n");
		assert (0);
	}
